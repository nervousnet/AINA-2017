...
public class QueryRich extends SQLiteOpenHelper {
    ...
    public ArrayList<SensorReading_v3> getAll(ConfigurationClass config) {
        ...
        ArrayList<SensorReading_v3> returnList = new ArrayList<>();

        // 3. go over each row, build sensor value and add it to list
        if (cursor.moveToFirst()) {
            do {
                SensorReading_v3 reading = new SensorReading_v3();
                reading.loadFromConfig(config);
